...
export default function RoutePlanningPage() {
  // Filter orders waiting for collection (from mobile app)
  const pendingOrders = useMemo(() => {
    return ordersData.filter(order =>
      order.status === 'En attente' || order.status === 'Confirmée'
    );
  }, []);
  ...
  // Get driver's circuit (all collections sorted by time)
  ...
  const handleDriverClick = (driverId: string) => {
    ...
    if (circuit.length === 0) {
      alert('Ce chauffeur n\'a pas encore de collectes planifiées');
      return;
    }
  ...
  const totalUnassigned = totalPendingOrders - totalAssigned;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Column - Pending Orders (2/3 width) */}
        <div className="lg:col-span-2 space-y-4">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Commandes à Collecter</CardTitle>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-ink-500 space-y-1">
                              <div className="flex items-center gap-4">
                                ...
                                <span>•</span>
                                <span>Taille: {order.estimatedSize}</span>
                                <span>•</span>
                    ...
                    >
                      ...
                      {assignedCount > 0 && (
                        <Badge variant="success" className="text-xs">
                          {assignedCount} collecte{assignedCount > 1 ? 's' : ''}
                        </Badge>
                      )}
                    </div>
                  );
            ...
            <CardContent className="flex-1 overflow-y-auto">
              {(() => {
                ...
                const totalCircuitWeight = circuit.reduce(
                  (sum, item) => sum + (item.order?.estimatedWeight || 0),
                  0
                );
                          ...
                          return (
                            <div key={assignment.orderId} className="relative pb-8">
                              ...
                              {index < circuit.length - 1 && (
                                <div className="absolute left-6 top-12 bottom-0 w-0.5 bg-terra-600" />
                              )}
